...
public class JdbcFilterRepository implements FilterRepository {
    ...
    private static final String INSERT = "INSERT INTO filters (name, link_id) VALUES (?, ?) ON CONFLICT DO NOTHING";
    private static final String EXISTS_BY_NAME = "SELECT COUNT(*) FROM filters WHERE name = ?";
    private static final String FIND_BY_NAME = "SELECT id, name, link_id FROM filters WHERE name = ?";
    private static final String DELETE = "DELETE FROM filters WHERE id = ?";
    private static final String DELETE_ALL_BY_LINK = "DELETE FROM filters WHERE link_id = ?";
    private static final String FIND_ALL_BY_LINK = "SELECT id, name, link_id FROM filters WHERE link_id = ?";

    @Override
    @Transactional
    public void saveAll(Collection<Filter> filters) {
        jdbc.batchUpdate(INSERT, filters, filters.size(), (ps, filter) -> {
    ...
    public void delete(Filter filter) {
        jdbc.update(DELETE, filter.getId());
    }
